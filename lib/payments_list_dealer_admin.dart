...
// this is 
class PaymentsListOfDealer_Admin extends StatefulWidget {
  // user object is sent from previous screen
  const PaymentsListOfDealer_Admin(this.dealerUser);
  final Dealers dealerUser;
  
  @override
  _PaymentsListOfDealer_AdminState createState() => _PaymentsListOfDealer_AdminState();
}

class _PaymentsListOfDealer_AdminState extends State<PaymentsListOfDealer_Admin> with TickerProviderStateMixin {
 ...
 // no profile exists
    ...
    void refreshUserPaymentUpdateAdmin1(BuildContext context) async {
      ...
      if(await checkInternetConnectivity()){
        ...
        // API call
        // print("${APIUrls.amount}${APIUrls.pass}/U3/${widget.dealerUser.dealerId}/$offset");
        var result = await get(Uri.parse(APIUrls.getUrl("${APIUrls.amount}${APIUrls.pass}/U3/${widget.dealerUser.dealerId}/$offset", queryParams)), headers: {"Accept": "application/json"});
        // print(result.body);
        ...
        if(jsonObject['status'] == 200){
            ...
            var paymentsData = jsonObject['data'] as List;
            // Map<String, dynamic> paymentsData = jsonObject['data'];

            if(paymentsData.isNotEmpty){
              // convert to list
              dealerPayments = paymentsData.map<Payments>((json) => Payments.fromJson(json)).toList();
              // sort invoices by invoice date
              // dealerPayments.sort((a,b) => DateTime.parse(a.paymentDate!).compareTo(DateTime.parse(b.paymentDate!)));
              
              // Calculate pending count and update each invoice
              double pendingCount = 0.0;
              // List<Payments> updatedList = dealerPayments.map((payment) {
              //   pendingCount += payment.amount!;
              ...
              setState(() {
                // Get new user data
                // invoices = paymentsData.where((element) => false);
                // invoices.
                double totalSum = dealerPayments.fold(0.0, (double sum, Payments payment) {
  ...
  void _scrollListener(){
    if(scrollController!.position.pixels == scrollController!.position.maxScrollExtent){
      setState(() {
        ...
        if(dealerPayments.length-5 == offset){
          offset = offset+5;
          // show up the loader
          startLoader();
        }
  ...
  void handleSelection(invoiceItem) {
    ...
    // else {
        ...
        if(invoiceItem.appliedAmount > 0){
            ...
            List<Payments> prevInvoices = List<Payments>.from(dealerPayments);

            // Update the invoices list
            prevInvoices = prevInvoices.map((invoice) {
              if (invoice.invoiceNo == invoiceItem.invoiceNo && invoice.amount! > 0) {
                // print("Amount applying: ${invoice.appliedAmount}");
                // print("Remaining: ${remainingCredit + invoice.appliedAmount!}");

                setState(() {
                  remainingCredit += invoice.amount!;
                });
          ...
          // Update the invoices list
          prevInvoices = prevInvoices.map((invoice) {
            if (invoice.invoiceNo == invoiceItem.invoiceNo && remainingCredit > 0 && invoice.amount == 0) {
              
              double applyAmount = (invoice.amount! < remainingCredit) ? invoice.amount! : remainingCredit;
  ...
  //   // const invoicesWithAppliedAmount = dealerPayments.filter(invoice => invoice.appliedAmount > 0);
  //   List<Payments> invoicesWithAppliedAmount = dealerPayments.where((invoice) => invoice.amount! > 0).toList();
                          ...
                          children: <Widget>[
                              ...
                              children: <Widget>[
                                ...
                                Row(
                                  ...
                                  Column(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      Text('Dealer Payments', style: GoogleFonts.inter(textStyle: Theme.of(context).textTheme.bodyLarge, fontSize: 20, fontWeight: FontWeight.bold, color:Color(0xFF008060)), ),
                                      sizedBox(4),
                            ...
                            Expanded(
                              ...
                              ListView.builder(
                                  ...
                                  itemCount: dealerPayments.length,
                              ...
                              child: Center(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    sizedBox(8),
                                    Text('No payments yet!', style: GoogleFonts.inter(textStyle: Theme.of(context).textTheme.bodyLarge, color: Colors.black54, fontWeight: FontWeight.w500, fontSize: 14), ),
                                  ],
                                )
                              )
                            ),
                            ...
                            dealerPayments.isNotEmpty ? Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  sizedBox(16),
                                  // show Sign in button
...
Widget paymentCard(int position){
  return InkWell(
    ...
    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => DealerDetails(id, name, dealerPayments[position].dealerId!, dealerPayments[position].accountName!))),
    child: Column(
    mainAxisAlignment: MainAxisAlignment.start,
    crossAxisAlignment: CrossAxisAlignment.start,
    children: <Widget>[
      ...
      // (dealerPayments[position].status == Constants.NotPaid) ?
      // Container(
      ...
      // sizedBox(8),
      // Text('${dealerPayments[position].accountName!}', textAlign: TextAlign.center, style: GoogleFonts.inter(textStyle: Theme.of(context).textTheme.bodyLarge, color: , )),
      Row(
        children: [
          Text('₹ ${NumberFormat("#,##,##0.00", "en_IN").format(dealerPayments[position].amount!)}', style: GoogleFonts.montserrat(textStyle: Theme.of(context).textTheme.bodyLarge, fontSize: 20, letterSpacing: 1.5, fontWeight: FontWeight.bold)),
          SizedBox(width: 4,),
          (dealerPayments[position].adminId != '-') ? 
          Container(
            padding: const EdgeInsets.fromLTRB(6, 2, 8, 3),
            // decoration: BoxDecoration(
            //         color: dealerPayments[position].type == 'credit' ? Color(0xFF01B11C) : Color(0xFFC41306),
            ...
            //     SizedBox(width: 4,),
            //     Text('${dealerPayments[position].type}' , style: GoogleFonts.inter(textStyle: Theme.of(context).textTheme.bodySmall, height: 1.5, color: Colors.white, fontWeight: FontWeight.bold ), overflow: TextOverflow.ellipsis, maxLines: 2, softWrap: true,  )
            //   ]
            // )
            (dealerPayments[position].adminId != '-') ? const Icon(PhosphorIconsFill.checkCircle, size: 24, color: Color(0xFFFFFFFF),) : 
            Row(children: [
          ...
          // SizedBox(width: 4,),
          // Text('${dealerPayments[position].type}', style: GoogleFonts.inter(textStyle: Theme.of(context).textTheme.bodySmall, fontWeight: FontWeight.w600, color: dealerPayments[position].type == 'credit' ? Colors.green : Color(0xFFC41306))),  
        ],
      ),
      // sizedBox(8),
      // Text('${dealerPayments[position].invoiceNo}', textAlign: TextAlign.center, style: GoogleFonts.inter(textStyle: Theme.of(context).textTheme.bodySmall, color: Colors.black54, )),
      
      Text('${DateFormat('d-MMM-y', 'en_US').format(getDate(dealerPayments[position].paymentDate!))}', textAlign: TextAlign.center, style: GoogleFonts.inter(textStyle: Theme.of(context).textTheme.bodySmall, color: Colors.black54, )),
      ...
      Row(
        ...
        children: [
          ...
          Column(
            ...
            children: [
              sizedBox(4),
              Text('${dealerPayments[position].invoiceNo}', textAlign: TextAlign.center, style: GoogleFonts.inter(textStyle: Theme.of(context).textTheme.bodySmall, color: Colors.black87, )),
              ...
              Row(
                ...
                children: [
                      ...
                      Expanded(child: 
                      Text('${dealerPayments[position].transactionId!}' , style: GoogleFonts.inter(textStyle: Theme.of(context).textTheme.bodySmall, color: Colors.black54), overflow: TextOverflow.ellipsis, maxLines: 4, softWrap: true, ),
                      )
                ]
              ),
              Row(
                ...
                spacing: 4,
                children: [
                      Text('Amount(s):' , style: GoogleFonts.inter(textStyle: Theme.of(context).textTheme.bodySmall, color: Colors.black87), overflow: TextOverflow.ellipsis, maxLines: 4, softWrap: true, ),
                      Expanded(child: 
                      Text('${dealerPayments[position].amounts!}' , style: GoogleFonts.inter(textStyle: Theme.of(context).textTheme.bodySmall, color: Colors.black54), overflow: TextOverflow.ellipsis, maxLines: 4, softWrap: true, ),
                      )
                ]
              ),
              Row(
                ...
                spacing: 4,
                children: [
                      Text('Invoice(s):' , style: GoogleFonts.inter(textStyle: Theme.of(context).textTheme.bodySmall, color: Colors.black87), overflow: TextOverflow.ellipsis, maxLines: 4, softWrap: true, ),
                      Expanded(child: 
                      Text('${dealerPayments[position].invoiceNo!}' , style: GoogleFonts.inter(textStyle: Theme.of(context).textTheme.bodySmall, color: Colors.black54), overflow: TextOverflow.ellipsis, maxLines: 4, softWrap: true, ),
                      )
                ]
              ),
              // sizedBox(4),
              // Text('${DateFormat('d-MMM-y', 'en_US').format(getDate(dealerPayments[position].paymentDate!))}', textAlign: TextAlign.center, style: GoogleFonts.inter(textStyle: Theme.of(context).textTheme.bodyMedium, color: Colors.black54, )),
              // Text('Paid on: ${DateFormat('d-MMM-y', 'en_US').format(getDate(dealerPayments[position].paymentDate!))}', textAlign: TextAlign.center, style: GoogleFonts.inter(textStyle: Theme.of(context).textTheme.bodyMedium, color: Colors.black54, )),
              sizedBox(4),
            ],
          ),
          ),
          ...
          //                       deletePayment(dealerPayments[position]);
          //                       Navigator.of(context).pop(); // Close the dialog
          //                     },
      ...
      //     SizedBox(width: 4,),
      //     Text('${DateFormat('MMM dd, yyyy · hh:mm aa', 'en_US').format(getDate(dealerPayments[position].expiryDate!))}', style: GoogleFonts.inter(textStyle: Theme.of(context).textTheme.bodySmall, color: Colors.black54)),
