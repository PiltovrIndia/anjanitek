...
class _HomePageState extends State<HomePage> {
  ...
  void getUserData() async {
    ...
    if (preferences.containsKey(constants.name)) {
      ...
      if (preferences.getString(constants.appVersionFromAPI) != null) {
        String appVersionFromAPI =
            preferences.getString(constants.appVersionFromAPI)!;
        if (appVersionFromAPI != constants.sc_app_version) {
          showToast(
              context, 'Please update to new version of the app.', 'warning');
          // show update dialog
          // showDialog(
          //   context: context,
      ...
      setState(() {
        ...
        destinations = allDestinations.where(_canShowDestination).toList();
  ...
  bool _canShowDestination(Destination destination) {
    if (destination.title == 'Orders') {
      return _normalizedRole != _normalizeRole(constants.factory);
    }

    return true;
  }

  String get _normalizedRole => _normalizeRole(role);

  String _normalizeRole(String value) {
    return value.toLowerCase().replaceAll(RegExp(r'[^a-z0-9]'), '');
  }

  bool get _usesAdminOrdersPage {
    const adminOrderRoles = {
      'admin',
      'globaladmin',
      'superadmin',
      'salesexecutive',
    ...
    return adminOrderRoles.contains(_normalizedRole);
  }
  ...
  void _handleNavigationTap(int index) {
    if (_pages == null || index < 0 || index >= destinations.length) {
      return;
    }
