...
class PaymentUpdateAdmin extends StatefulWidget {
  const PaymentUpdateAdmin(this.id, this.name, this.invoiceNo);
  final String id, name, invoiceNo;

  @override
  _PaymentUpdateAdminState createState() => _PaymentUpdateAdminState();
  
}

class _PaymentUpdateAdminState extends State<PaymentUpdateAdmin> with TickerProviderStateMixin {
  ...
  // on subitting values
    ...
    void submitPayment(BuildContext context) async {
      ...
      // print("${APIUrls.submitPayment}${APIUrls.pass}/${creditAmount.trim()}/$verifyOTP/$_uuid/SMART/${DateFormat('yyyy-MM-dd HH:mm:ss', 'en_US').format(today)}");
      // print("${APIUrls.payments}${APIUrls.pass}/mobile/${widget.id}/${creditAmount.trim()}/credit/-/$transactionId/${DateFormat('yyyy-MM-dd HH:mm:ss', 'en_US').format(fromDate1)}/$adminId/-");
      // var result = await get(Uri.parse(APIUrls.getUrl("${APIUrls.payments}${APIUrls.pass}/mobile/${widget.id}/${creditAmount.trim()}/credit/-/$transactionId/${DateFormat('yyyy-MM-dd HH:mm:ss', 'en_US').format(fromDate1)}/$adminId/-", queryParams)), headers: {"Accept": "application/json"});
      // print("${APIUrls.payments}${APIUrls.pass}/mobile/${widget.id}/${creditAmount.trim()}/credit/${Uri.encodeComponent(widget.invoiceNo)}/$transactionId/${DateFormat('yyyy-MM-dd HH:mm:ss', 'en_US').format(fromDate1)}/$adminId/-");
      var result = await get(Uri.parse(APIUrls.getUrl("${APIUrls.payments}${APIUrls.pass}/mobile/${widget.id}/${creditAmount.trim()}/credit/${Uri.encodeComponent(widget.invoiceNo)}/$transactionId/${DateFormat('yyyy-MM-dd HH:mm:ss', 'en_US').format(fromDate1)}/$adminId/-", queryParams)), headers: {"Accept": "application/json"});
      // print(result.body);
                  ...
                  child:
                      ...
                      Column(
                        ...
                        children: [
                          ...
                          sizedBox(4),
                          Text('for', style: GoogleFonts.inter(textStyle: Theme.of(context).textTheme.bodySmall, fontWeight: FontWeight.bold), ),
                          sizedBox(4),
                    ...
                    Container(
                      margin: const EdgeInsets.fromLTRB(32, 8, 32, 8),
                            ...
                            child: Column(children: <Widget>[
                              Text('Credit amount will be considered for the pending invoices', style: GoogleFonts.inter(textStyle: Theme.of(context).textTheme.bodyMedium),),
