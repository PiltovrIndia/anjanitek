...
class StockReservationsAdmin extends StatefulWidget {
  const StockReservationsAdmin({super.key});

  @override
  _StockReservationsAdminState createState() => _StockReservationsAdminState();
}

class _StockReservationsAdminState extends State<StockReservationsAdmin> {
  ...
  Future<List<ReservationProduct>> _fetchReservations() async {
    ...
    final uri = Uri.parse(APIUrls.getUrl("${APIUrls.reservations}${APIUrls.pass}/U0/$selectedStatus/$offset/$role/$id/$sortBy", {}));
  ...
  void _replaceReservationInState(ReservationProduct updatedReservation) {
    final reservationIndex = _reservations.indexWhere(
      (item) => item.id == updatedReservation.id,
    );
  ...
  Widget _buildReservationCard(BuildContext context, ReservationProduct reservation, {bool isGrouped = false}) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          (reservation.status.toLowerCase() == 'submitted' && id == reservation.userId) ? sizedBox(12) : sizedBox(0),
          (reservation.status.toLowerCase() == 'submitted' && id == reservation.userId) ? DottedLine() : sizedBox(0),
          (reservation.status.toLowerCase() == 'submitted' && id == reservation.userId) ? sizedBox(12) : sizedBox(0),
          (reservation.status.toLowerCase() != 'submitted' && id == reservation.userId && role.toLowerCase() == Constants.superAdmin.toLowerCase())
          // (reservation.status.toLowerCase() == 'approved' ||
          //         reservation.status.toLowerCase() == 'rejected')
              ? ElevatedButton(
                  style: ButtonStyle(
                  ...
                  : ElevatedButton(
                      onPressed: () async {
                        ...
                        final result = await get(
                          Uri.parse(APIUrls.getUrl("${APIUrls.reservations}${APIUrls.pass}/U5/${reservation.id}", {})),
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          if (reservations.length == 1) {
            return _buildReservationCard(context, reservations.first, isGrouped: true);
          }
                ...
                if (isExpanded) ...[
                  ...
                  ...reservations.asMap().entries.map((entry) {
                    ...
                    return Padding(
                      ...
                      child: _buildReservationCard(context, reservation, isGrouped: false),
                    );
                  }),
                ],
              ],
  ...
  void _showApprovalBottomSheet(BuildContext context, ReservationProduct reservation) {
                        ...
                        child: isRejecting
                            ...
                            : ElevatedButton(
                                onPressed: () async {
                                  ...
                                  print(
                                      "${APIUrls.reservations}${APIUrls.pass}/$path/${reservation.id}/Rejected/0/${reservation.userId}/$expiryStr");
                                  final result = await get(
                                  ...
                                  if (jsonObject['status'] == 200) {
                                    _replaceReservationInState(
                                      ReservationProduct(
                                        ...
                                        isProduction: reservation.isProduction,
                                    ...
                                    showToast(context, 'Stock rejected.',
                                        Constants.success);
                        ...
                        child: isApproving
                            ...
                            : ElevatedButton(
                                onPressed: () async {
                                  ...
                                  final result = await get(
                                    Uri.parse(APIUrls.getUrl(
                                        "${APIUrls.reservations}${APIUrls.pass}/$path/${reservation.id}/${path == 'U3.1' ? 'Modified' : 'Approved'}/${_approvedQtyController.text.trim()}/${reservation.userId}/$expiryStr",
                                        {})),
                                    headers: {"Accept": "application/json"},
                                  );
                                  var jsonString = jsonDecode(result.body);
          ...
          children: [
            ...
            Container(
              ...
              child: DropdownButton<String>(
                ...
                value: sortBy,
                isExpanded: true,
                items: sorts.map((Map<String, dynamic> sort) {
              ...
              child: Container(
                ...
                child: TabBar(
                  ...
                  tabs: [
                    ...
                    Tab(text: 'Future orders (${futureOrders.length})'),
                  ],
                ),
              ),
            ),
