...
import 'package:anjanitek/utils/deviceuuid.dart';
...
class _VerificationState extends State<Verification>
  ...
  Future<void> initPlatformState() async {
    ...
    try {
      uuid = await DeviceUUID.getDeviceUUID();
  ...
  void verifyUser(BuildContext context) async {
    try {
      ...
      // print(
      //     "${APIUrls.verifyUser}${APIUrls.pass}/${mobileNumber.trim()}/$verifyOTP/$_uuid/${DateFormat('yyyy-MM-dd HH:mm:ss', 'en_US').format(today)}");
      var result = await get(
          Uri.parse(Uri.encodeFull(APIUrls.getUrl(
              "${APIUrls.verifyUser}${APIUrls.pass}/${mobileNumber.trim()}/$verifyOTP/$_uuid/${DateFormat('yyyy-MM-dd HH:mm:ss', 'en_US').format(today)}",
              queryParams))),
      ...
      if (jsonObject['status'] == 200) {
        ...
        var result1 = await get(
            Uri.parse(Uri.encodeFull(
                "https://messaging.charteredinfo.com/smsaspx?ID=piltovrindia@gmail.com&Pwd=PiltovrIndia@33&PhNo=91" +
                    mobileNumber.trim() +
                    "&Text=" +
                    verifyOTP +
                    " is OTP for Anjani Tek login. Valid for 5 minutes. Regards, Team Piltovr.&TemplateID=1007619515128741928")),
            headers: {"Accept": "application/json"});
        ...
        setState(() {
          ...
          if (Users.fromJson(userdata).role!.toLowerCase() ==
        ...
        // updatePlayerId(user);
          ...
          jsonObject['status'] == 402 ||
          jsonObject['status'] == 404 ||
  ...
  void verifyOTPNow(BuildContext context) {
    if (formKey.currentState!.validate()) {
      ...
      if (verifyOTP == submittedOTP) {
        ...
        // update player Id
        updatePlayerId(user!);
      } else if ((mobileNumber == 'A33' && submittedOTP == '1234') ||
          (mobileNumber == 'S33' && submittedOTP == '1234') ||
          (mobileNumber == 'OA33' && submittedOTP == '1234') ||
          (mobileNumber == 'OAS33' && submittedOTP == '1234')) {
        setState(() {
          verifyOTPLoading = true;
        });
  ...
  Future<void> updatePlayerId(Users user) async {
    ...
    if (playerId != null) {
      if (playerId.length > 2) {
        ...
        var result = await get(
            Uri.parse(Uri.encodeFull(APIUrls.getUrl(
                "${APIUrls.user}/${APIUrls.pass}/U1/${user.id}/$playerId",
                queryParams))),
            headers: {"Accept": "application/json"});
        ...
        if (jsonObject['status'] == 200) {
          ...
          if (val) {
            ...
            setState(() {
              isLoading = false;
              errorMsg =
                  'Sorry, your account is not created yet! Please contact your campus administrator';
            });
          }
        } else {
          setState(() {
              ...
              children: <Widget>[
                ...
                Container(
                    ...
                    child: Column(
                      children: <Widget>[
                        Text(
                          'Contact your sales person incase you have issues to login to the app',
                          style: GoogleFonts.inter(
                              textStyle:
